...
def predict():
    try:
        ...
        # use the same method and mapping used during training.
        # if you used a label encoder or one hot encoder, use it here.
        # if you did not encode, and just used integer values, this section will need to be changed to that method.
        # example of label encoder usage.
        # input_data = encoder.transform([features])
        # example of no encoding, but integer based input.
        # input_data = np.array([int(feature) for feature in features]).reshape(1, -1)
        # if you used a mapping dictionary, use it here.
        # example of mapping dictionary use.
        # mapping = {'a':0, 'b':1, ....} #example, replace with your mapping.
        # input_data = np.array([mapping[feature] for feature in features]).reshape(1, -1)
        # if you did one hot encoding, you will need to do it here.
